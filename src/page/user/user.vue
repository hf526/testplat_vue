<template>
  <div>
    ...
    <el-card class="box-card">
      ...
      <!--添加用户-->
        <el-dialog title="添加用户" :visible.sync="dialogFormVisible" :show-close=false center width="28%" class="addupdialog">
            <el-form ref="form" :rules="userrules" :model="userform" label-position="left" label-width="70px" style="margin-bottom: -20px;margin-right: 5px" >
      ...
      <!--编辑用户-->
      <el-dialog title="编辑用户" :visible.sync="edituser" :show-close=false center width="28%" class="addupdialog">
        <el-form ref="form" :rules="userrules" :model="eduserform" label-position="left" label-width="70px" style="margin-bottom: -20px;margin-right: 5px" >
          <el-form-item label="用户名" prop="username">
...
export default {
  ...
  methods: {
    ...
    // 查询获取用户列表
    async getuserlist () {
      await this.$get(this.$apilist.userlist, this.queryinfo)
        .then(res => {
    ...
    async chargestatus (userinfo) {
      await this.$post(this.$apilist.upuserstatus, userinfo)
        .then(res => {
